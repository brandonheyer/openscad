...
module makeNpt(sizes = NPT_LOOKUP[0], internal = false, test = $preview, extraLength = 0, length) {
  diameter = sizes[0];
  tpi = sizes[1];
  l = length ? length : sizes[2] + extraLength;
...
module npt(size = 0.75, internal = false, test = $preview, extraLength = 0, length) {
  if (size == 0.75) {
    makeNpt(NPT_LOOKUP[0], internal, test, extraLength, length);
  } else if (size == 1) {
    makeNpt(NPT_LOOKUP[1], internal, test, extraLength, length);    
  }
}

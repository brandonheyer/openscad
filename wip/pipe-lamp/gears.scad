...
module spurArm(zahnbreite, schritt, rho_ra, rb) {
  #polygon(
    concat(
      [[0,0]],
      [
        for (rho = [0 : schritt : rho_ra - schritt])		
          pol_zu_kart(ev(rb, rho))
      ],
      [
        for (rho = [rho_ra - schritt : -schritt : 0])	
          pol_zu_kart([
            ev(rb,rho)[0], 
            zahnbreite-ev(rb,rho)[1]
          ])
       ]
    )
  );
...
module stirnrad(
  ...
  optimiert = false
	...
	r = d / 2;						
	alpha_stirn = atan(
    tan(eingriffswinkel) / cos(schraegungswinkel)
  );
	...
	gamma = rad * breite / (r * tan(90 - schraegungswinkel));
	schritt = rho_ra / 8;											
	tau = 360 / zahnzahl;											
	r_loch = (2*rf - bohrung)/8;									
	rm = bohrung/2+2*r_loch;										
	z_loch = floor(2*pi*rm/(3*r_loch));					

  difference() {
    union() {
      rotate([0, 0, -phi_r - 90 * (1 - spiel) / zahnzahl]) {				
      // TODO  
         ...
         linear_extrude(height = breite, twist = gamma)  {
          ...
          for (rot = [0 : tau : 359]) {
            rotate(rot)
              spurArm(zahnbreite, schritt, rho_ra, rb);
          }
        }		
      }
      
      cylinder(breite, r = rm + r_loch * 1.51);
    }
...
module pfeilrad(
  ...
  optimiert=false
    ...
    difference() {
      union() {
        cylinder(breite * 2, r = rm + r_loch * 1.51);
        cylinder(breite * 2 + nabendicke, r = nabendurchmesser / 2);  
      }
      
      translate([0, 0, -breite / 2]) 
        cylinder((breite * 2 + nabendicke) * 2, r = bohrung / 2);
    }
	}
}
...
module planetengetriebe(modul, zahnzahl_sonne, zahnzahl_planet, anzahl_planeten, breite, randbreite, bohrung, eingriffswinkel=20, schraegungswinkel=0, zusammen_gebaut=true, optimiert=true){
	...
	// Dimensions-Berechnungen
	d_sonne = modul * zahnzahl_sonne;										
	d_planet = modul * zahnzahl_planet;									
	achsabstand = modul * (zahnzahl_sonne + zahnzahl_planet) / 2;		
	zahnzahl_hohlrad = zahnzahl_sonne + 2 * zahnzahl_planet;
  d_hohlrad = modul * zahnzahl_hohlrad;									
  drehen = istgerade(zahnzahl_planet);
		
	n_max = floor(
    180 / asin(
      d_planet/(modul * (zahnzahl_sonne +  zahnzahl_planet))
    )
  );
	...
	rotate([0,0,180 / zahnzahl_sonne * drehen]) {
      ...
      schraegungswinkel=-schraegungswinkel, 
      optimiert=optimiert
    );		// Sonnenrad
	}
    ...
    if(anzahl_planeten == 0) {
      list = [
      for (n=[2 : 1 : n_max]) if ((((zahnzahl_hohlrad+zahnzahl_sonne)/n)==floor((zahnzahl_hohlrad+zahnzahl_sonne)/n))) n];
        anzahl_planeten = list[0];			
		...
		for(i=[-(anzahl_planeten-1):2:(anzahl_planeten-1)]){
			translate([planetenabstand, d_planet*i,0])
				pfeilrad (modul, zahnzahl_planet, breite, bohrung, nabendicke=0, nabendurchmesser=0, eingriffswinkel=eingriffswinkel, schraegungswinkel=schraegungswinkel, optimiert = false);	// Planetenräder
		}
	}
